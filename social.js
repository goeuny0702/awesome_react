...
import socialData from './assets/서울시 사회복지시설(청소년복지시설) 목록.json';
...
const Social = ({ route, navigation }) => {
  ...
  // 해당 구에 대한 시설 정보를 필터링하여 처리
  const filteredData = district
    ? socialData.DATA.filter(item => item.구 === district).map(item => ({
        ...item,
        lat: item.lat || (parseFloat(item.Dlat) + 0.001),  // 시설의 lat 사용
        lng: item.lng || (parseFloat(item.Dlng) + 0.001), // 시설의 lng 사용
      }))
    : socialData.DATA.map(item => ({
        ...item,
        lat: item.lat || (parseFloat(item.Dlat) + 0.001),
        lng: item.lng || (parseFloat(item.Dlng) + 0.001),
      }));
  ...
  const mapHtml = `<!DOCTYPE html>
    ...
    <body>
      ...
      <script>
        var initialCenter = new kakao.maps.LatLng(${route.params?.centerLat || 37.5665}, ${route.params?.centerLng || 126.9780});
...
kakao.maps.event.addListener(map, 'click', function() {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>{district || '전체'} 복지시설</Text>
      ...
      <TouchableOpacity
        style={[styles.navButton, { bottom: 90 }]} // 상단 버튼 (예: Road)
      ...
      <TouchableOpacity
        style={[styles.navButton, { bottom: 20 }]} // 하단 버튼 (예: Home)
